...
def create_weapon(weapon: schemas.WeaponCreate, db: Session = Depends(get_db)):
    ...
    p = influxdb_client.Point("backend_measurement").tag("total_time", "weapon").field("get_weapon", str(time.time() - start_time))
...
def read_weapon(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    p = influxdb_client.Point("backend_measurement").tag("total_time", "weapon").field("create_weapon", str(time.time() - start_time))
    write_api.write(bucket=bucket, org=org, record=p)
    return weapons
...
def delete_weapon(weapon_id: int, db: Session = Depends(get_db)):
    ...
    p = influxdb_client.Point("backend_measurement").tag("total_time", "weapon").field("delete_weapon", str(time.time() - start_time))
    write_api.write(bucket=bucket, org=org, record=p)
...
def update_weapon(weapon_id: int, weapons: schemas.WeaponUpdate, db: Session = Depends(get_db)):
    ...
    p = influxdb_client.Point("backend_measurement").tag("total_time", "weapon").field("patch_weapon", str(time.time() - start_time))
    write_api.write(bucket=bucket, org=org, record=p)
    return db_weapon
...
def change_weapon(weapon: schemas.WeaponCreate, weapon_id: int, db: Session = Depends(get_db)):
    ...
    p = influxdb_client.Point("backend_measurement").tag("total_time", "weapon").field("put_weapon", str(time.time() - start_time))
    write_api.write(bucket=bucket, org=org, record=p)
    return db_weapon
